...
/// \param key 实验的键名
///
/// \param callback 回调函数，返回实验配置（字典）或空字典
///
- (void)getExperiment:(NSString * _Nonnull)key callback:(void (^ _Nonnull)(NSDictionary<NSString *, id> * _Nonnull))callback;
/// 获取功能配置（A/B 测试）
/// \param key 功能配置的键名
///
/// \param callback 回调函数，返回功能配置（可能是字典、字符串等类型）
///
- (void)getFeatureConfig:(NSString * _Nonnull)key callback:(void (^ _Nonnull)(id _Nonnull))callback;
@end
